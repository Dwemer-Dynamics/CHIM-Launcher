...
class DwemerDistroLauncher(tk.Tk):
    ...
    def ensure_stobeserver_repo_exists(self, target_branch="stobe"):
        ...
        result = self.run_wsl_bash_capture(
            ...
            "rm -rf \"$repo_path\" || { echo ERROR:REPO_CLEAN_FAILED:$repo_path >&2; exit 1; }; "
            "mv \"$legacy_path\" \"$repo_path\" || { echo ERROR:MIGRATE_FAILED:$legacy_path >&2; exit 1; }; "
            ...
            "if [ ! -d \"$repo_path/.git\" ]; then "
            "rm -rf \"$repo_path\" || { echo ERROR:REPO_CLEAN_FAILED:$repo_path >&2; exit 1; }; "
            f"git clone -b {normalized_branch} https://github.com/Dwemer-Dynamics/StobeServer.git \"$repo_path\" 1>&2 || "
            f"{{ echo ERROR:CLONE_FAILED:branch={normalized_branch}:path=$repo_path >&2; exit 1; }}; "
            f"state=CLONED:{normalized_branch}; "
            "fi; "
            "fi; "
            "mkdir -p \"$repo_path/log\" || { echo ERROR:LOG_DIR_FAILED:$repo_path/log >&2; exit 1; }; "
            ": > \"$repo_path/log/stobe_import.log\" || { echo ERROR:LOG_RESET_FAILED:$repo_path/log/stobe_import.log >&2; exit 1; }; "
            ": > \"$repo_path/log/stobeserver.log\" || { echo ERROR:LOG_RESET_FAILED:$repo_path/log/stobeserver.log >&2; exit 1; }; "
            "echo \"$state\""
        )
...
"""
    ...
    def view_parakeet_logs(self):
        """Opens a new terminal window to view the Parakeet logs."""
        cmd = 'wsl -d DwemerAI4Skyrim3 -u dwemer -- tail -n 100 -f /home/dwemer/parakeet-api-server/log.txt'
        threading.Thread(target=self.run_command_in_new_window, args=(cmd,), daemon=True).start()
